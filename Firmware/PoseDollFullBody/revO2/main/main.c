/* Rev O G0 + six remote nodes. PD41 v1 remains unchanged; no calibration, UDP, IMU or OTP writes.
...
static void gateway(void){
    ...
    uint64_t serial_seq=0,next=clock_us(),last_announce=0,last_maintenance=0;rx_event_t e;
    unsigned cursor=0;bool missing[6]={true,true,true,true,true,true};
    for(;;){
        ...
        if(atomic_exchange(&rx_overflow,false) || cohort.sequence==UINT32_MAX){
            can_reset();pd_init(&cohort,random_id());last_announce=0;
            for(unsigned i=0;i<6;i++)missing[i]=true;
        }
        bool ok=true;unsigned target=o2_select_maintenance(&cohort,missing,&cursor,clock_us(),last_maintenance);
        ...
        if(!ok){
            can_reset();pd_init(&cohort,random_id());last_announce=0;join=(o2_join_t){0};
            for(unsigned i=0;i<6;i++)missing[i]=true;
        ...
        pd_sample_t sample;if(pd_finish(&cohort,clock_us(),&sample)){
            if(!target)for(unsigned i=0;i<6;i++)missing[i]=(sample.presence&(1u<<i))==0;
