...
RHO={'frame':1.24,'shell':1.24,'metal':7.85,'fastener':7.85,'spring':7.85,'bush':1.41,'lining':1.30,'thrust':7.85}
def main():
 results={}
 reff=(2/3)*(12.5**3-4.15**3)/(12.5**2-4.15**2)
 for name in ('manny','quinn'):
  p,parts,meta=build(name)
  data=json.loads((ROOT/f'verification/revI_{name}.json').read_text('utf8'))
 ...
 result=dict(status='ASSUMPTION_BUDGET_NOT_LOAD_RATING',densities_g_cm3_assumed=RHO,
  assumptions=['Printed geometry treated as solid PLA, not a slicer mass','All metal geometry treated as steel, including unconfirmed inherited materials',
   'Unmodeled hand/wrist provision is a 100g point at wrist, not a measured part','Reservation volumes omitted',
  ...
  variants=results)
 h.save(ROOT/'verification/revI_load_budget.json',result)
