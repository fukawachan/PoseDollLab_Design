...
def main():
 for name in ('manny','quinn'):
  ...
  assy=cq.Assembly(name=name+'_RevJ_shoulder_assembly');rows=[]
  for item in neutral:
   ...
   if any(x in item['name'] for x in ('_clutch_','_drive_','_abduct_sensor_')) or item['name'] in meta['changed_frames']:
  ...
  h.save(ROOT/f'verification/revJ_{name}.json',record)
  h.render([(q['name'],q['shape'],COL[q['material']]) for q in neutral if q['role']!='reservation'],folder/'neutral.png',name.title()+' | Rev J shoulder drive + actual PCB integration',camera=(1000,-1500,600))
