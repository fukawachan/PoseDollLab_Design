...
def main():
  ...
  for side,sy in [('l',1),('r',-1)]:
   ...
   for label,normal,xd,base,tip in [('flex',(0,-sy,0),(1,0,0),43,-24),('abduct',(1,0,0),(0,0,1),26,-16)]:
    ...
    def omit(q):
     ...
     return q['name'].startswith(side+'_abduct_sensor_') or q['name'] in (side+'_twist_bush',side+'_twist_shaft')
   ...
   check('rear_journal_radial_M2',tool,lambda q:False,prefix+'journal_radial_M2x4')
   def board_removed(q):
 ...
 h.save(ROOT/'verification/revJ_tool_access.json',dict(scope='Neutral staged geometry, 2 mm M2 driver shaft / 3 mm M3 shaft; no grip, torque or insertion sweep test',checks=results))
