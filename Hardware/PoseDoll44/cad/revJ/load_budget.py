...
RHO={'frame':1.24,'shell':1.24,'metal':7.85,'fastener':7.85,'spring':7.85,'bush':1.41,'lining':1.30,'thrust':7.85,'aluminum':2.70,'brass':8.50,'nylon':1.14,'magnet':7.50,'pcb':1.85,'pcb_component':3.0}
def main():
 results={}
 reff=(2/3)*(12.5**3-4.15**3)/(12.5**2-4.15**2)
 for name in ('manny','quinn'):
  p,parts,meta=build(name)
  data=json.loads((ROOT/f'verification/revJ_{name}.json').read_text('utf8'))
 ...
 result=dict(status='ASSUMPTION_BUDGET_NOT_LOAD_RATING',densities_g_cm3_assumed=RHO,
  assumptions=['Printed geometry treated as solid PLA, not a slicer mass','Inherited metal geometry treated as steel; new hubs/journals as aluminum, radial screws as brass; materials unqualified',
  ...
  variants=results)
 h.save(ROOT/'verification/revJ_load_budget.json',result)
