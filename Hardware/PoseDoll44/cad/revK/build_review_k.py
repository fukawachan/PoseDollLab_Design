...
def main():
 ...
 for name in ('manny','quinn'):
  p,parts,meta=build(name);record=json.loads((ROOT/f'verification/revK_{name}.json').read_text('utf8'))
  ...
  h.render([(q['name'],q['shape'],COL[q['material']]) for q in detail],OUT/name/'shoulder_detail.png',
   name.title()+' | Rev K actual CAD | left shoulder',camera=(1000,-1500,600))
  ...
  h.render([(q['name'],q['shape'],COL[q['material']]) for q in items if q['role']!='reservation'],OUT/name/'crossed_clearance_candidate.png',
   name.title()+' | Rev K staggered arm pose | sampled clearance',camera=(1800,-600,200))
 (OUT/'RevK_Shoulder_Review.html').write_text((HERE/'review.template.html').read_text('utf8').replace('__DATA__',json.dumps(data,separators=(',',':'))),encoding='utf8')
 h.save(ROOT/'verification/revK_viewer_geometry.json',counts)
