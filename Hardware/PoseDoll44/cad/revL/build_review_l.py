...
from flex_encoder import poses_all
...
def main():
 ...
 for name in ('manny','quinn'):
  p,parts,meta=build(name);record=json.loads((ROOT/f'verification/revL_{name}.json').read_text('utf8'))
  ...
  for item in parts:
   ...
   assert np.linalg.norm(np.array(item['local_shape'].Center().toTuple())-np.array(verified[item['name']]['local_com_mm']))<1e-5,item['name']
   # Discard fine render triangulation left by an earlier cached build stage.
   # This only clears mesh caches; analytical BRep geometry is unchanged.
   BRepTools.Clean_s(item['local_shape'].wrapped)
  ...
  h.render([(q['name'],q['shape'],COL[q['material']]) for q in detail],OUT/name/'shoulder_detail.png',
   name.title()+' | Rev L actual CAD | left shoulder',camera=(1000,-1500,600))
  ...
  h.render([(q['name'],q['shape'],COL[q['material']]) for q in items if q['role']!='reservation'],OUT/name/'crossed_clearance_candidate.png',
   name.title()+' | Rev L staggered arm pose | sampled clearance',camera=(1800,-600,200))
 (OUT/'RevL_Shoulder_Review.html').write_text((HERE/'review.template.html').read_text('utf8').replace('__DATA__',json.dumps(data,separators=(',',':'))),encoding='utf8')
 h.save(ROOT/'verification/revL_viewer_geometry.json',counts)
