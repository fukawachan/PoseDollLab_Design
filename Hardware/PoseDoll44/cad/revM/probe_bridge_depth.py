...
for name in ('quinn','manny'):
 ...
 rows=[]
 for depth in (32.,30.,28.,34.,26.,22.):
  shape=rails([C+[-57,10,35],C+[-57,35,35],C+[65,35,35],C+[65,0,35],[C[0]+65,0,N[2]-depth],N+[15,0,-depth],N+[15,0,-30],N+[6.3,0,-30]],3) if depth!=30 else rails([C+[-57,10,35],C+[-57,35,35],C+[65,35,35],C+[65,0,35],[C[0]+65,0,N[2]-30],N+[6.3,0,-30]],3)
  bad=[]
  ...
  rows.append(dict(depth=depth,structural=bad));print(json.dumps(dict(character=name,depth=depth,structural=len(bad),first=bad[:5])),flush=True);h.save(ROOT/'verification/revM_bridge_depth_work.json',out|{name:rows})
  if not bad:break
 out[name]=rows
