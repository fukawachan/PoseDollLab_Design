...
for name in ('quinn','manny'):
 ...
 for c in channels41(A,name):
  ...
  sh=cube((9.4,5.5,6.5),(0,-.4,z+3.25)).rotate((0,0,0),(0,0,1),c.get('board_rotation_deg',0)).moved(loc)
  replace_world(A,q,sh,'Candidate JST SH6 mated plug enclosure, no cable bend yet','pcb_component');changed.append(q['name'])
 ...
 out[name]=rows;h.save(ROOT/'verification/revM_direct_sensor_JST_probe.json',out);print(name,'completed',len(rows),'failing',sum(any(q['classification']=='structural' for q in r['review']) for r in rows),flush=True)
