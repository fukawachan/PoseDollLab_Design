...
for name in ('quinn','manny'):
 ...
 out[name]=rows;h.save(ROOT/'verification/revM_side_sensor_JST_probe.json',out);print(name,'completed',len(rows),'failing',sum(any(q['classification']=='structural' for q in r['review']) for r in rows),flush=True)
