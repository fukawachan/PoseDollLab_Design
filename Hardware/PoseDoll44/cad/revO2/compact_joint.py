"""L6-R2: split bearing cartridge, floating two-face brake and real threaded cap.
...
from revo2_evidence import read,save,sha,run_paths
THREAD_PITCH=.75
...
def module(lighten=True):
 ...
 cup=ring(15,12.35,-13.4,1.5).fuse(ring(15,4.3,0,1.5))
 cup=cup.cut(cyl(13.70,-13.5,-4.7))
 cup=cup.fuse(ring(7.49,4.50,1.5,2.0))
 for x in (-12.4,12.4):cup=cup.cut(box(1.2,2.2,7.5,(x,0,-2.0)))
 ...
 cup=cup.fuse(box(4.0,4.0,4.2,(16.7,0,-9.5))).cut(hole_x(.83,13.0,19,0,-9.5))
 for x in (-10.7,10.7):cup=cup.fuse(cyl(.30,-.5,.05).translate((x,0,0)))
 assert male.isValid() and female.isValid(), 'Helical tools must not self-intersect'
 before=cup.Volume();cup=cup.cut(female)
 ...
 add('floating_brake_disc',rotor,'steel','child',note='Free axial key length covers +/-0.6 mm study travel; no shoulder in brake load path.')
 front=ring(12,4.2,-.6,0)
 for x in (-10.7,10.7):front=front.cut(cyl(.31,-.61,.01).translate((x,0,0)))
 add('front_friction_lining',front,'lining')
 rear=ring(12,4.2,-2.7,-2.1);wear=ring(12,4.3,-2.9,-2.7)
 plate=ring(12,4.3,-4.7,-2.9)
 for x in (-12.4,12.4):plate=plate.fuse(box(.9,2.0,1.8,(x,0,-3.8)))
 for x in (-10.7,10.7):
  rear=rear.cut(cyl(.31,-2.71,-2.09).translate((x,0,0)))
  wear=wear.cut(cyl(.31,-2.91,-2.69).translate((x,0,0)))
  plate=plate.fuse(cyl(.30,-2.95,-2.3).translate((x,0,0)))
 ...
 add('rear_friction_lining',rear,'lining');add('wear_sheet',wear,'steel');add('keyed_pressure_plate',plate,'al6061')
 add('spring_front_washer',ring(6.03,3.15,-5.2,-4.7),'steel')
 for i in range(4):add('disc_spring_'+str(i+1),disc_spring(-7.55+i*.5875,.5875,i%2==1),'steel',note='SCHNORR 002100 geometry at catalog 0.75 h0; force/tolerance curve not qualified.')
 add('spring_rear_washer',ring(6.03,3.15,-8.05,-7.55),'steel')
 ...
 add('threaded_adjuster_cap',cap,'al6061',note='Diameter 28 x 0.75 custom 60-degree reference 60-degree thread; 4.0 mm axial overlap, production tolerance class pending supplier.')
 lock=hole_x(.79,15.0,18.7,0,-9.5).fuse(hole_x(.48,13.41,15.0,0,-9.5)).fuse(hole_x(1.7,18.7,20.3,0,-9.5))
 add('cap_lock_dog_screw',lock,'steel',note='Positive slot dog, not friction-only cap lock. Small fastener threads represented by root core plus reference engagement.')
 ...
 for x in (-7.7,7.7):
  post=ring(1.5,.65,20,pcbdatum-.91).translate((x,0,0))
  shelf=box(3.4,2.5,.5,(x/abs(x)*6.5,0,pcbdatum-1.16))
  add('sensor_post_'+str(x),post.fuse(shelf).cut(cyl(.65,19.99,pcbdatum-.90).translate((x,0,0))),'al6061')
 ...
 tools={'hollow_pin_spanner':tool.translate((0,0,.4))}
 return rows,tools
