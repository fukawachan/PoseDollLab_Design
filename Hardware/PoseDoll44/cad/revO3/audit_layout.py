"""Derive O3 occupied volumes from actual KiCad exports and explicit connector frames.
...
def main():
    ...
    for board in er['boards']:
        ...
        for c in layout['components']:
            ...
            if c['ref'] in missing:
                ...
                if len(fab):
                    lo=fab.min(axis=0);hi=fab.max(axis=0);height=.8 if c['value'].startswith('SN74LVC125') else 5.0;top=-layout['board_mm'][2] if c['back'] else height;bottom=top-height if c['back'] else 0
                    substitute=bound_box([lo[0],-hi[1],bottom,hi[0],-lo[1],top]);add(c['ref']+'_missing_model_envelope','physical_envelope_unqualified',substitute,'Actual footprint F.Fab XY; TI BQA 0.8 mm max height where identified, otherwise unresolved 5 mm sensitivity assumption','NOT_A_COMPLETE_QUALIFIED_COMPONENT_MODEL')
            ...
            n=14 if c['ref']=='J50' else 6;side='_SM' in c['footprint'];width=n+1
            if side:
    ...
    save(v/'pcba_geometry.json',{'schema':'revo3-PCBA-geometry-v1','boards':reports,'consumed_inputs':reader.receipt(),'status':'PARTIAL_DERIVED_GEOMETRY','JST_CAD_download_status':'Official STEP request requires personal/company details and emailed delivery; not submitted. Catalog reference envelope used openly.','physical_tested':False})
