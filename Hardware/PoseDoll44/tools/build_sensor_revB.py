...
components=[
 ...
 dict(ref="J1",lib="Connector_Generic",sym="Conn_01x06",value="SM06B-GHS-TB",xy=(40,70),pcb=(109, 103.4),fp="Connector_JST:JST_GH_SM06B-GHS-TB_1x06-1MP_P1.25mm_Horizontal",
 ...
 dict(ref="C2",lib="Device",sym="C",value="10u X7R 10V",xy=(170,65),pcb=(114.7, 112.3),fp="Capacitor_SMD:C_0805_2012Metric",nets={"1":"+3V3","2":"GND"}),
...
def schematic():
    libs={};body=[];rootid=uid("schematic");name="PoseDoll_AS5048A_revB"
    for c in components:
        key=c["lib"]+":"+c["sym"]
...
def board(rootid):
    b=pcb.BOARD();nets={}
    subprocess.run([str(LIB.parents[1]/"bin/kicad-cli.exe"),"sch","export","netlist","--format","kicadxml","-o",str(OUT/"netlist.xml"),str(OUT/"PoseDoll_AS5048A_revB.kicad_sch")],check=True)
    netmap={}
    ...
    label=pcb.PCB_TEXT(b);label.SetText("PD44 S-B");label.SetPosition(pcb.VECTOR2I(pcb.FromMM(109),pcb.FromMM(117.8)));label.SetTextSize(pcb.VECTOR2I(pcb.FromMM(.8),pcb.FromMM(.8)));label.SetLayer(pcb.F_SilkS);b.Add(label)
    from route_sensor_revB import route
    route(b)
    pcb.SaveBoard(str(OUT/"PoseDoll_AS5048A_revB.kicad_pcb"),b)
