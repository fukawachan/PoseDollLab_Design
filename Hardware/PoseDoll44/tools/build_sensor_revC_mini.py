...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/"electronics/sensor_revC_mini"
...
components=[
 ...
 dict(ref="J1",lib="Connector_Generic",sym="Conn_01x06",value="FH19C-6S-0.5SH(10)",xy=(40,70),pcb=(100, 100),fp="PoseDoll:Hirose_FH19C_6S_0.5SH",
 ...
 dict(ref="C2",lib="Device",sym="C",value="10u X7R 10V",xy=(170,65),pcb=(102.3, 103.75),fp="Capacitor_SMD:C_0805_2012Metric",nets={"1":"+3V3","2":"GND"}),
...
def schematic():
    libs={};body=[];rootid=uid("schematic");name="PoseDoll_AS5048A_revC_mini"
    for c in components:
        key=c["lib"]+":"+c["sym"]
...
def board(rootid):
    ...
    subprocess.run([str(LIB.parents[1]/"bin/kicad-cli.exe"),"sch","export","netlist","--format","kicadxml","-o",str(OUT/"netlist.xml"),str(OUT/"PoseDoll_AS5048A_revC_mini.kicad_sch")],check=True)
    netmap={}
    ...
    pcb.SaveBoard(str(OUT/"PoseDoll_AS5048A_revC_mini.kicad_pcb"),b)
...
if __name__=="__main__":
    ...
    from sensor_revC_mini_rules import configure
    configure()
    print(OUT)
