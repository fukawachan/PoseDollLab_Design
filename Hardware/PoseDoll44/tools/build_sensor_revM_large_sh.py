...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/"electronics/sensor_revM_large_sh"
OUT.mkdir(parents=True,exist_ok=True)
LIB=Path(r"D:/ProgramFiles/KiCad/10.0/share/kicad")
NS=uuid.UUID("ab43272d-3a19-45b4-b3fc-ecb7bb302ab0")
...
components=[
 dict(ref="U1",lib="Sensor_Magnetic",sym="AS5048A",value="AS5048A-HTSP-500",xy=(90,75),pcb=(109, 110),fp="Package_SO:TSSOP-14_4.4x5mm_P0.65mm",
      nets={"1":"CS_N","2":"SCK","3":"MISO_IC","4":"MOSI","5":"GND","6":None,"7":None,"8":None,"9":None,"10":None,"11":"+3V3","12":"+3V3","13":"GND","14":None}),
 dict(ref="J1",lib="Connector_Generic",sym="Conn_01x06",value="SM06B-SRSS-TB(LF)(SN)",xy=(40,70),pcb=(109, 103.4),fp="Connector_JST:JST_SH_SM06B-SRSS-TB_1x06-1MP_P1.00mm_Horizontal",
      nets={"1":"GND","2":"+3V3","3":"SCK","4":"MOSI","5":"MISO","6":"CS_N"}),
 dict(ref="C1",lib="Device",sym="C",value="100n X7R 16V",xy=(150,65),pcb=(114.7, 108.6),fp="Capacitor_SMD:C_0603_1608Metric",nets={"1":"+3V3","2":"GND"}),
 dict(ref="C2",lib="Device",sym="C",value="10u X5R 25V / GRM219R61E106KA12D",xy=(170,65),pcb=(114.7, 112.3),fp="Capacitor_SMD:C_0805_2012Metric",nets={"1":"+3V3","2":"GND"}),
 dict(ref="R1",lib="Device",sym="R",value="33R",xy=(150,100),pcb=(103, 109.35),fp="Resistor_SMD:R_0603_1608Metric",nets={"1":"MISO_IC","2":"MISO"}),
 dict(ref="R2",lib="Device",sym="R",value="10k",xy=(170,100),pcb=(103, 112.4),fp="Resistor_SMD:R_0603_1608Metric",nets={"1":"+3V3","2":"CS_N"}),
...
def schematic():
    libs={};body=[];rootid=uid("schematic");name="PoseDoll_AS5048A_revM_large_sh"
    for c in components:
        key=c["lib"]+":"+c["sym"]
...
def board(rootid):
    b=pcb.BOARD();nets={}
    subprocess.run([str(LIB.parents[1]/"bin/kicad-cli.exe"),"sch","export","netlist","--format","kicadxml","-o",str(OUT/"netlist.xml"),str(OUT/"PoseDoll_AS5048A_revM_large_sh.kicad_sch")],check=True)
    netmap={}
    ...
    for c in components:
        ...
        if c["ref"]=="J1":f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(109),pcb.FromMM(119)))
        if c["ref"]=="U1":f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(109),pcb.FromMM(114.2)))
        if c["ref"]=="R1":f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(101.1),pcb.FromMM(108)))
        if c["ref"]=="R2":
            f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(101.1),pcb.FromMM(111.1)))
            f.Reference().SetTextSize(pcb.VECTOR2I(pcb.FromMM(.8),pcb.FromMM(.8)))
        footprints[c["ref"]]=f
    for i,(x,y) in enumerate(((102.5,116.5),(115.5,116.5)),1):
        f=pcb.FootprintLoad(str(LIB/"footprints/MountingHole.pretty"),"MountingHole_2.2mm_M2")
        f.SetAttributes(f.GetAttributes()|pcb.FP_BOARD_ONLY);f.SetFPID(pcb.LIB_ID("MountingHole","MountingHole_2.2mm_M2"));f.SetReference("H"+str(i));f.SetPosition(pcb.VECTOR2I(pcb.FromMM(x),pcb.FromMM(y)));f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(x),pcb.FromMM(y+1.8)));b.Add(f)
    for a,z in (((100,100),(118,100)),((118,100),(118,120)),((118,120),(100,120)),((100,120),(100,100))):
    ...
    label=pcb.PCB_TEXT(b);label.SetText("PD41 SH-L");label.SetPosition(pcb.VECTOR2I(pcb.FromMM(109),pcb.FromMM(117.8)));label.SetTextSize(pcb.VECTOR2I(pcb.FromMM(.8),pcb.FromMM(.8)));label.SetLayer(pcb.F_SilkS);b.Add(label)
    from route_sensor_revM_large_sh import route
    import os
    os.environ["PD44_ROUTE_ORDER"]="MISO_IC,SCK,MOSI,+3V3,CS_N,MISO,GND"
    route(b)
    b.GetDesignSettings().SetAuxOrigin(pcb.VECTOR2I(pcb.FromMM(109),pcb.FromMM(110)))
    pcb.SaveBoard(str(OUT/"PoseDoll_AS5048A_revM_large_sh.kicad_pcb"),b)
...
if __name__=="__main__":
    ...
    pro=json.loads((ROOT/'electronics/sensor_revB/PoseDoll_AS5048A_revB.kicad_pro').read_text(encoding='utf8'));pro['meta']['filename']='PoseDoll_AS5048A_revM_large_sh.kicad_pro';pro['board']['design_settings']['rules'].update(min_track_width=.15,min_clearance=.15);pro['net_settings']['classes'][0].update(track_width=.15,clearance=.15);(OUT/pro['meta']['filename']).write_text(json.dumps(pro,indent=2),encoding='utf8')
    print(OUT)
