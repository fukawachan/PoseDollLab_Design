...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/"electronics/sensor_revM_sh"
...
components=[
 ...
 dict(ref="J1",lib="Connector_Generic",sym="Conn_01x06",value="BM06B-SRSS-TB(LF)(SN)",xy=(40,70),pcb=(100, 100),fp="Connector_JST:JST_SH_BM06B-SRSS-TB_1x06-1MP_P1.00mm_Vertical",
 ...
 dict(ref="C2",lib="Device",sym="C",value="10u X5R 25V / CL21A106KAYNNNE",xy=(170,65),pcb=(102.3, 103.75),fp="Capacitor_SMD:C_0805_2012Metric",nets={"1":"+3V3","2":"GND"}),
...
def schematic():
    libs={};body=[];rootid=uid("schematic");name="PoseDoll_AS5048A_revM_sh"
    for c in components:
        key=c["lib"]+":"+c["sym"]
...
def board(rootid):
    ...
    subprocess.run([str(LIB.parents[1]/"bin/kicad-cli.exe"),"sch","export","netlist","--format","kicadxml","-o",str(OUT/"netlist.xml"),str(OUT/"PoseDoll_AS5048A_revM_sh.kicad_sch")],check=True)
    netmap={}
    ...
    pcb.SaveBoard(str(OUT/"PoseDoll_AS5048A_revM_sh.kicad_pcb"),b)
