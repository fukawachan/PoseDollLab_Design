...
STEP=.1
XLO,XHI,YLO,YHI=1007,1173,1007,1193
...
def route(b):
    ...
    def free(state,net):
        x,y,l=state
        return XLO<=x<=XHI and YLO<=y<=YHI and not (occupancy[l].get((x,y),set())-{net})
    def viaok(x,y,net):
    ...
    logs=[]
    order=[int(x) for x in os.environ.get("PD44_ROUTE_ORDER","5,7,6,1,2,4,3").split(",")]
    for net in order:
