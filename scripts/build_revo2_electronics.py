...
from revo2_evidence import REPO,HW,run_paths,read,save,sha
...
def main():
    ...
    for kind,dims in [('proximal5',(36,48)),('distal4',(26,30)),('distal4_narrow',(20,36))]:
        directory=out/'electronics'/kind;directory.mkdir(parents=True,exist_ok=True)
        name='PoseDoll_RevO2_'+kind;legacy.OUT=directory;legacy.NAME=name;legacy.comps=components('proximal5' if kind=='proximal5' else 'distal4')
        ...
        lib=directory/'RevO2.pretty';lib.mkdir(exist_ok=True)
        for f in b.GetFootprints():
            if f.GetReference()=='U1':
                f.SetFPID(pcb.LIB_ID('RevO2','ESP32-S3-WROOM-1_thermal030'));pcb.PCB_IO_MGR.FindPlugin(pcb.PCB_IO_MGR.KICAD_SEXP).FootprintSave(str(lib),f)
        ...
        (directory/'fp-lib-table').write_text('(fp_lib_table (lib (name RevO2) (type KiCad) (uri ${KIPRJMOD}/RevO2.pretty) (options "") (descr "0.3 mm drill / 0.65 mm pad thermal candidate; qualification pending")))',encoding='utf-8')
    ...
    save(verify/'electronics.json',{'schema':'revo2-electronics-v1','boards':reports,'expected_new_board_types':['proximal5','distal4','distal4_narrow'],'distal_options_are_alternatives_not_extra_nodes':True,'logical_node_mapping':partition,'physical_acquisition_pcb_count':8,'CAN_physical_nodes':7,'measured_axes':41,'library_input_sha256':library_hashes,'toolchain':{'version':version,'kicad_cli_sha256':sha(CLI),'pcbnew_version':pcb.Version()},'status':'PLACEMENT_STUDY_NOT_ROUTED','physical_tested':False})
