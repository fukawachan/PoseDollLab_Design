...
def main():
 verify,out=run_paths();rid=os.environ['REVO2_RUN_ID'];root=REPO/'.local/revo2'/rid/'firmware';fw=REPO/'Firmware/PoseDollFullBody/revO2';reports=[]
 ...
 for role in ['G0','N1','N2','N3','N4','N5','N6']:
  ...
  if proc.returncode==0 and binary.is_file() and config.is_file():
   ...
   result.update(status='PASS' if valid else 'FAIL_ROLE_CONFIG',config_role_checked=valid,binary=str(binary.relative_to(REPO)),binary_sha256=sha(binary),sdkconfig_sha256=sha(config),binary_bytes=binary.stat().st_size)
