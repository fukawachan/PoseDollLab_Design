"""O4 reference-surface packaging, driven only by sealed generated PCBA bounds.
...
def main():
    v,o=run_paths();pr=ArtifactReader('pcba_geometry');jr=ArtifactReader('baseline');pcba=pr.json(v/'pcba_geometry.json');joint=jr.json(v/'joint.json');boards={q['kind']:q for q in pcba['boards']};chars=[];scenes={}
    ...
    for char in ('manny','quinn'):
        ...
        for side in ('l','r'):
            ...
            for variant,radius in itertools.product(('A_distal4','distal4_rs485','distal4_rs485_stretch','distal4_rs485_top'),('4','12')):
        ...
        torso=[]
        for variant,radius in itertools.product(('A_proximal5','proximal5_rs485'),('4','12')):
    ...
    save(v/'packaging.json',{'schema':'revo4-derived-packaging-v1','characters':chars,'status':'BLOCKED_FULL_MULTIXIS_PCBA_AND_HARNESS','bend_radii_mm':[4,12],'radii_are_not_qualified':True,'RF_keepout_retained':True,'mass_override_does_not_remove_size_checks':True,'consumed_inputs':[pr.receipt(),jr.receipt()]})
