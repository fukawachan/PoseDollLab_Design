"""O4 run evidence: seal every produced file, then verify the full set before/after consumption."""
from pathlib import Path
import datetime, hashlib, json, os, re, subprocess, sys
from revo2_evidence import REPO, HW, sha, read, git_head
VERIFY = HW / 'verification/revO4'
GENERATED = HW / 'generated/revO4'
class BlockedInput(RuntimeError):
    pass

def save(path, value):
    path = Path(path); path.parent.mkdir(parents=True, exist_ok=True)
    if isinstance(value, dict) and os.environ.get('REVO4_RUN_ID'):
        value = {'run_id': os.environ['REVO4_RUN_ID'], **value}
    path.write_text(json.dumps(value, ensure_ascii=False, indent=2, allow_nan=False)+'\n', encoding='utf-8')

def sources(include_ue=True):
    from revo2_evidence import sources as old_sources
    result = old_sources()
    for folder in (REPO/'Firmware/PoseDollFullBody/revO4', HW/'docs/revO4_review_source'):
    ...
    if not include_ue:return dict(sorted(result.items()))
    ue=Path('E:/UnrealProjects/DollSimulation')
    for folder in (ue/'Plugins/PoseDoll/Source',ue/'Tools/PoseDollSimulator/src',ue/'Tools/PoseDollSimulator/tests',ue/'Shared/Profiles',ue/'Shared/Fixtures/O4'):
        for p in folder.rglob('*'):
            if p.is_file() and '__pycache__' not in p.parts and p.suffix!='.pyc':result['@ue/'+p.relative_to(ue).as_posix()]=sha(p)
    for p in (ue/'Scripts').glob('*o4*.py'):result['@ue/'+p.relative_to(ue).as_posix()]=sha(p)
    return dict(sorted(result.items()))

def run_paths(run_id=None):
    rid = run_id or os.environ.get('REVO4_RUN_ID')
    if not rid or not re.fullmatch(r'[A-Za-z0-9_-]+', rid):
        raise ValueError('Explicit safe REVO4_RUN_ID required')
...
def seal_artifacts(path, producer, run_id, roots):
    ...
    manifest={'schema':'revo4-artifact-set-v1','run_id':run_id,'producer':producer,'roots':roots,'files':files,'merkle_root':merkle(files)}
    save(path,manifest)
...
class ArtifactReader:
    ...
    def __init__(self, producer):
        deps=json.loads(os.environ.get('REVO4_DEPENDENCIES','{}'))
        if producer not in deps: raise BlockedInput('No verified producer supplied: '+producer)
        self.record=deps[producer];self.data=verify_artifacts(self.record,os.environ.get('REVO4_RUN_ID'));self.used={}
    def path(self, path):
    ...
    def receipt(self):
        verify_artifacts(self.record,os.environ.get('REVO4_RUN_ID'))
        return {'producer_manifest':self.record,'actually_read_sha256':self.used}

class Run:
    def __init__(self, run_id, include_ue=True):
        ...
        self.include_ue=include_ue;self.inputs=sources(include_ue);self.steps={}
        self.meta={'schema':'revo4-run-v1','run_id':run_id,'base_commit':git_head(),'started_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source_sha256':self.inputs,'input_scope':'hardware_and_UE' if include_ue else 'hardware_only_UE_tested_separately','physical_tested':False,'manufacturing_released':False}
    ...
    def step(self,name,command,dependencies=(),extra_env=None,expected_paths=(),artifact_roots=()):
        ...
        try:
            ...
            env=os.environ.copy();env.update(REVO4_RUN_ID=self.run_id,PYTHONUTF8='1',REVO4_DEPENDENCIES=json.dumps(deps));env.update(extra_env or {})
    ...
    def finish(self):
        current=sources(self.include_ue)
